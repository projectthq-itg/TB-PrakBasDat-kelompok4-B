...
if (!$hewan || $hewan['pelanggan_id'] != $pelanggan_id) {
    header('Location: hewan_saya.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama_hewan) || empty($jenis_hewan)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE hewan_peliharaan SET nama_hewan = ?, jenis_hewan = ?, ras = ?, umur = ?, berat = ?, warna = ? WHERE id = ? AND pelanggan_id = ?");
            $stmt->execute([$nama_hewan, $jenis_hewan, $ras, $umur, $berat, $warna, $id, $pelanggan_id]);

            addNotifikasi($pelanggan_id, 'Hewan Diperbarui', "Data hewan $nama_hewan telah diperbarui.", 'info');
            ...
            <div class="form-container">
                ...
                <form method="POST">
                    <div class="form-group">
                        ...
                        <input type="text" name="nama_hewan" value="<?php echo htmlspecialchars($hewan['nama_hewan']); ?>" required>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select name="jenis_hewan" required>
                                ...
                                <option value="Burung" <?php echo $hewan['jenis_hewan'] === 'Burung' ? 'selected' : ''; ?>>Burung</option>
                                <option value="Hamster" <?php echo $hewan['jenis_hewan'] === 'Hamster' ? 'selected' : ''; ?>>Hamster</option>
                                <option value="Ikan" <?php echo $hewan['jenis_hewan'] === 'Ikan' ? 'selected' : ''; ?>>Ikan</option>
                                <option value="Reptil" <?php echo $hewan['jenis_hewan'] === 'Reptil' ? 'selected' : ''; ?>>Reptil</option>
                                <option value="Lainnya" <?php echo $hewan['jenis_hewan'] === 'Lainnya' ? 'selected' : ''; ?>>Lainnya</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Ras</label>
                            <input type="text" name="ras" value="<?php echo htmlspecialchars($hewan['ras'] ?? ''); ?>" placeholder="Contoh: Persia, Golden Retriever">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            <label>Umur (tahun)</label>
                            <input type="number" name="umur" min="0" max="50" value="<?php echo $hewan['umur'] ?? 0; ?>">
                        </div>
                        <div class="form-group">
                            <label>Berat (kg)</label>
                            <input type="number" name="berat" min="0" step="0.1" value="<?php echo $hewan['berat'] ?? 0; ?>">
                        </div>
                    </div>
                    <div class="form-group">
                        <label>Warna</label>
                        <input type="text" name="warna" value="<?php echo htmlspecialchars($hewan['warna'] ?? ''); ?>" placeholder="Contoh: Putih, Hitam, Coklat">
                    </div>
